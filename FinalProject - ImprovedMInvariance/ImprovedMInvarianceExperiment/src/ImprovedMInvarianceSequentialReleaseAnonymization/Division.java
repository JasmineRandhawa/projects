...
public class Division {
	...
	public List<EmployeeHealthCareData> CreateCommonDataSet(List<EmployeeHealthCareData> dPrevious,
		...
		for (EmployeeHealthCareData current : dCurrent) {
			for (EmployeeHealthCareData previous : dPrevious)
				// extract the tuples which have same employeeId and same signature (list of distinct
	...
	public List<EmployeeHealthCareData> CreateDisjointDataSets(List<EmployeeHealthCareData> dPrevious,
		...
		List<String> previousEmployeeIds = dPrevious.stream().map(EmployeeHealthCareData::getEmployeeId).map(String::toUpperCase)
				.collect(Collectors.toList());
		for (EmployeeHealthCareData current : dCurrent) {
			// check in current data for new rows with different EmployeeIds from previous data
			if (!previousEmployeeIds.contains(current.EmployeeId.toUpperCase()))
				dDisjoint.add(current);

			// check in current data for rows with same EmployeeIds and different Occupation from
			// previous data
			else {
				EmployeeHealthCareData previousOccupationForSameEmployee = dPrevious.stream()
						.filter((previous) -> (previous.EmployeeId.toUpperCase().equals(current.EmployeeId.toUpperCase())
								&& !previous.Occupation.toUpperCase().equals(current.Occupation.toUpperCase())))
	...
	public List<EmployeeHealthCareInfo> CreateBuckets(List<EmployeeHealthCareData> dCommon,
			List<EmployeeHealthCareInfo> dPreviousAnonymized) {
		// match the signature of these two data sets and create buckets based on EmployeeId
		...
		for (EmployeeHealthCareInfo prevAnonymized : dPreviousAnonymized) {
			List<Integer> bucketGroupIds = buckets.stream().map(EmployeeHealthCareInfo::getGroupId)
					.collect(Collectors.toList());

			if (!bucketGroupIds.contains(prevAnonymized.GroupId)) {
				...
				for (EmployeeHealthCareData common : dCommon) {
					...
					List<EmployeeHealthCareData> commonSignatureRows = prevAnonymized.GroupInfo.stream()
							...
							.collect(Collectors.toList());
					if (commonSignatureRows!=null && commonSignatureRows.stream().count() > 0) {
				...
				if (bucket.GroupInfo.stream().count()>0)  {
					...
					List<String> commonSensitiveAttributes = bucket.GroupInfo.stream()
							.map(EmployeeHealthCareData::getOccupation).map(String::toUpperCase).collect(Collectors.toList());
